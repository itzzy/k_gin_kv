...
#nohup python kgin_kv_train_r10_zzy.py --config config_kgin_kv_r10_zzy.yaml > log_kgin_kv_r10_0515_zzy.txt 2>&1 &
...
class TrainerAbstract:
    def __init__(self, config):
        ...
        if self.debug: config.general.exp_name = 'test_kgin_kv_r10_zzy'
...
class TrainerKInterpolator(TrainerAbstract):
    ...
    def run_test(self):
        ...
        with torch.no_grad():
            ...
            # 打印结果
            print(f'\nkgin_kv_r10_zzy:Validation PSNR - Mean: {psnr_mean:.4f} ± {np.sqrt(psnr_var):.4f} | Variance: {psnr_var:.4f}')
            ...
            # np.save('out_kgin_kv_0404_r10_zzy.npy', out)
            np.save('out_kgin_kv_0515_r10_zzy.npy', out)

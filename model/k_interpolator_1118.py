...
'''
本代码是最早的k_interpolator.py  但是运行报错
运行本代码会报以下错误，还未解决这个错误    
Traceback (most recent call last):
  ...
  File "/data0/zhiyong/code/github/itzzy_git/k-gin_kv/model/k_interpolator.py", line 331, in forward
    img_0F[:,i,:,:] = img_temp[:,i:i+5,:,:].sum(dim=1, keepdim=True) / mask_temp[:,i:i+5,:].sum(dim=1, keepdim=True)
RuntimeError: expand(CUDAComplexFloatType{[32, 32, 192, 192]}, size=[32, 192, 192]): the number of sizes provided (3) must be greater or equal to the number of dimensions in the tensor (4)
'''
...
class KInterpolator(nn.Module):
    def __init__(self, config):
        ...
        self.pos_embed = nn.Parameter(torch.zeros(1, self.ss*self.img_size[1] + 1, self.embed_dim), requires_grad=False)  # fixed sin-cos embedding
        ...
        self.decoder_pos_embed = nn.Parameter(torch.zeros(1, self.ss*self.img_size[1] + 1, self.decoder_embed_dim), requires_grad=False)  # fixed sin-cos embedding
    ...
    def initialize_weights(self):
        ...
        pos_embed = get_2d_sincos_pos_embed(self.pos_embed.shape[-1], self.ss, self.img_size[1], cls_token=True) #self.img_size[0]
        self.pos_embed.data.copy_(torch.tensor(pos_embed).unsqueeze(0))

        decoder_pos_embed = get_2d_sincos_pos_embed(self.decoder_pos_embed.shape[-1], self.ss, self.img_size[1], cls_token=True) #self.img_size[0]
...
#        img_temp = torch.cat([img[:,-2:,:,:], img, img[:,:2,:,:]], dim=1)
#        mask_temp = torch.cat([mask[:,-2:,:], mask, mask[:,:2,:]], dim=1)
        img_temp = torch.cat([img, img[:,-5:,:,:]], dim=1)
        mask_temp = torch.cat([mask, mask[:,-5:,:]], dim=1)
        img_0F = torch.zeros_like(img)
        mask_0F = torch.zeros_like(mask)
        for i in range(T):
            img_0F[:,i,:,:] = img_temp[:,i:i+5,:,:].sum(dim=1, keepdim=True) / mask_temp[:,i:i+5,:].sum(dim=1, keepdim=True)
            mask_0F[:,i,:] = mask_temp[:,i:i+5,:].sum(dim=1, keepdim=True)
        
        #img_0F = img_0F.repeat(1, 1, 1, 1)
        #img_0F = img_0F.repeat(1, self.ss, 1, 1)
        #torch.mean(img, dim=1, keepdim=True).repeat(1, img.shape[1], 1, 1)
                
        #mask_0F = torch.ones(mask.shape[0], mask.shape[1], mask.shape[2])
        #mask_0F = torch.ones(mask.shape[0], self.ss, mask.shape[2])
        ...
        for time in range(T):
            ...
            mask_0Ft = mask_0F[:, time:time+self.ss, :].flatten(1, -1)             
            img_0Ft = img_0F[:, :, time:time+self.ss, :]
            
            maskt = mask[:, time:time+self.ss, :].flatten(1, -1)             
            imgt = img[:, :, time:time+self.ss, :]
            #print('k, m', img_0Ft.shape, mask_0Ft.shape) #k, m torch.Size([1, 384, 1, 192]) torch.Size([1, 192])
            #print('k, m', imgt.shape, maskt.shape) #k, m torch.Size([1, 384, 1, 192]) torch.Size([1, 192])
            
            kv, _ = self.encoder(img_0Ft, mask_0Ft)            
            q, ids_restore = self.encoder(imgt, maskt)
            #print('k, q', kv.shape, q.shape)
            ...
            predt = torch.mean(predt.view(-1, self.ss, w, h_2), dim=1, keepdim=True)
            predt_list.append(predt)
        pred = torch.cat(predt_list, dim=1)    
        #predd = torch.cat(predt_list, dim=1)
        #mean_tensor = predd[:, 1:-1, :, :].view(1, -1, self.ss, w, h_2).mean(dim=2)
        #pred = torch.cat([predd[:, 0:1, :, :], mean_tensor, predd[:, -1:, :, :]], dim=1)

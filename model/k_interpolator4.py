...
forward-img_0F-2 type: <class 'torch.Tensor'>, shape: torch.Size([32, 128, 192, 192])
forward-img_0F-3 type: <class 'torch.Tensor'>, shape: torch.Size([32, 384, 128, 192])
  0%|                                                                                                                                                                    | 0/2 [00:13<?, ?it/s]
Traceback (most recent call last):
  ...
  File "/data0/zhiyong/code/github/itzzy_git/k-gin_kv/model/k_interpolator.py", line 357, in forward
    kv, _ = self.encoder(img_0F, mask_0F)
  File "/data0/zhiyong/code/github/itzzy_git/k-gin_kv/model/k_interpolator.py", line 203, in encoder
    kspace = kspace + self.pos_embed[:, 1:, :]
RuntimeError: The size of tensor a (24576) must match the size of tensor b (768) at non-singleton dimension 1
wandb:
wandb: You can sync this run to the cloud by running:
wandb: wandb sync /data0/zhiyong/code/github/itzzy_git/k-gin_kv/wandb/offline-run-20241118_111131-rd35vx7w
wandb: Find logs at: wandb/offline-run-20241118_111131-rd35vx7w/logs
'''
...
#        imgadd = torch.cat((mean_of_third_dim, tensor), dim=2)
    ...
    def forward(self, img, mask):
        ...
        print(f"forward-img_0F-3 type: {type(img_0F)}, shape: {img_0F.shape}")
        b, h_2, t, w = img.shape
        predt_list = []
        img = torch.cat([img, img[:,:,0:self.ss,:]], dim=2)
        mask = torch.cat([mask, mask[:,0:self.ss,:]], dim=1)
        for time in range(T):
